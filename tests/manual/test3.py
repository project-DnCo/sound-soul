...
async def add(
    ...
    # first: app_commands.Transform[int, Gt(5)],
...
async def graph(
    ...
    # library to use it.
    # point: Annotated[Point, PointTransformer],
...
@client.tree.command()
async def datacls(interaction: discord.Interaction, point: Annotated[Point4D, Point4D]) -> None:
    await interaction.response.send_message(str(point))
